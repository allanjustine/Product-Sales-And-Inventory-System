<div wire:poll.keep-alive.15s>
    ...
    <div class="dropdown" wire:ignore.self>
        <a class="float-right mt-4 mr-4 p-2 cartdropdown" id="cart-dropdown" data-toggle="dropdown"
            ...
            <span class="badge badge-pill badge-danger" id="badge-cart"><span style="font-size: 12px;">{{
                    $carts->count() }}</span></span>
        </a>
        <ul id="myDiv" class="dropdown-menu dropdown-menu-end cartmenu" style="z-index: 1049;"
            ...
            <li class="cart-item px-3 py-2">
                <div class="cart-item-image">
                    <button class="btn btn-link text-primary" wire:click.prevent="decreaseQuantity({{ $item->id }})"
                        onclick="handleButtonClick(event, {{ $item->id }})">
                        <i class="fas fa-minus text-black"></i>
                    </button>
                    x{{ number_format($item->quantity) }}
                    <button class="btn btn-link text-primary" wire:click.prevent="updateCartItem({{ $item->id }})"
                        onclick="handleButtonClick(event, {{ $item->id }})">
                ...
                <div class="cart-item-price mt-2">
                    ...
                    <span><strong>Sub total:
                            &#8369;{{ number_format($this->getProductTotalAmount($item->product_id), 2, '.', ',')
                            }}</strong></span>
                </div>
